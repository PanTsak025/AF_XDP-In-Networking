...
static int parse_packet_to_meta(uint8_t *pkt, uint32_t len, struct packet_meta *m)
{
    uint8_t *data = pkt;
...
static void classify_batch_cpu(struct packet_meta *batch, uint32_t count)
{
    for (uint32_t i = 0; i < count; i++) {
        if (!batch[i].valid)
            continue;

        update_source_counter(batch[i].src_ip);

        struct ip_counter *entry = get_ip_counter(batch[i].src_ip);
        if (entry && entry->malicious)
...
static int flush_classification_batch(struct xsk_state *xsks,
                                      struct packet_meta *meta_batch,
                                      ...
                                      uint32_t *batch_count)
{
    ...
    classify_batch_cpu(meta_batch, *batch_count);

    if (recycle_batch_addrs(xsks, recycle_addrs, *batch_count) < 0)
        return -1;
...
static void rx_loop(struct xsk_state *xsks)
{
    ...
    struct packet_meta meta_batch[CLASSIFY_BATCH_SIZE];
    ...
    while (running) {
        ...
        if (rcvd > 0) {
            for (unsigned int i = 0; i < rcvd; i++) {
                ...
                /* Start timing when first packet enters an empty batch */
                if (batch_count == 0 && !batch_timer_active) {
                    batch_start_ns = get_time_ns();
                    batch_timer_active = 1;
                }

                recycle_addrs[batch_count] = desc->addr;

                /* Keep invalid packets too so recycling stays aligned */
                (void)parse_packet_to_meta(pkt, desc->len, &meta_batch[batch_count]);

                batch_count++;

                /* Flush immediately when batch becomes full */
                if (batch_count == CLASSIFY_BATCH_SIZE) {
                    unsigned int flushed_pkts = batch_count;

                    if (flush_classification_batch(xsks, meta_batch, recycle_addrs, &batch_count) < 0) {
        ...
        /* Flush partial batch once per second under low traffic */
        if (batch_count > 0 && (now - last_print >= 1)) {
            unsigned int flushed_pkts = batch_count;
    ...
    if (batch_count > 0) {
        ...
        (void)flush_classification_batch(xsks, meta_batch, recycle_addrs, &batch_count);
...
int main(int argc, char **argv)
{
    ...
    if (load_and_attach_xdp("./xdp_sock_kern.o", ifname, &obj, &xsks.xsk_map_fd) < 0) {
    ...
    printf("AF_XDP CPU-batched running on if=%s queue=%d threshold_pps=%d rx_burst=%d classify_batch=%d\n",
